...
var settings = {
	...
	"region": process.env.S3_ENDPOINT_REGION || undefined,
	"bucket": process.env.S3_UPLOADS_BUCKET || undefined,
	"host": process.env.S3_UPLOADS_HOST || undefined,
	"aliasHost": process.env.S3_ALIAS_HOST || undefined,
	"accessGrant": process.env.S3_ACCESS_GRANT || undefined,
	"path": process.env.S3_UPLOADS_PATH || undefined
};

function fetchSettings(callback) {
	// set your storj data in this code block
	settings.accessKeyId = process.env.AWS_ACCESS_KEY_ID || "your access key";
	settings.secretAccessKey = process.env.AWS_SECRET_ACCESS_KEY || "your secret access key";
	settings.bucket = process.env.S3_UPLOADS_BUCKET || "your bucket name";
	settings.host = process.env.S3_UPLOADS_HOST || "gateway.storjshare.io";
	settings.aliasHost = process.env.S3_ALIAS_HOST || "link.storjshare.io/raw";
	settings.accessGrant = process.env.S3_ACCESS_GRANT || "your access grant key";
	settings.path = process.env.S3_UPLOADS_PATH || "";
	settings.endpoint = process.env.S3_ENDPOINT || "gateway.storjshare.io";
	settings.region = process.env.S3_ENDPOINT_REGION || "";
...
plugin.activate = function (data) {
	if (data.id === 'nodebb-plugin-storj-upload') {
		fetchSettings();
	}
...
function uploadToS3(filename, err, buffer, callback) {
	...
	S3().putObject(params, function (err) {
		...
		host = "https://" + settings.aliasHost + "/" + settings.accessGrant + "/" + settings.bucket; //forming the URL
